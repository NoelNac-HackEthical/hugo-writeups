...
description: "Découverte web automatisée via feroxbuster + ffuf avec heuristique de bruit (ffuf) et agrégation des chemins filtrés."
draft: false
tags: ["scripts","tools"]
...
Découverte web automatisée via feroxbuster + ffuf avec heuristique de bruit (ffuf) et agrégation des chemins filtrés.

## Présentation

mon-nouveau-recoweb — Découverte web ciblée pour CTF / pentest (feroxbuster + ffuf)

Ce script automatise la reconnaissance web d'une cible (ex. mon-site.htb) en combinant :
  - feroxbuster : scan récursif en premier, avec ses propres mécanismes internes (auto-filter, recursion, etc.),
  - ffuf        : scan guidé par baseline + heuristique de “bruit de fond” (404/403 génériques), en second.
...
- Pour chaque port HTTP/HTTPS détecté :
  ...
  3) Post-traitement et agrégation globale :
     - Lecture de tous les chemins trouvés par feroxbuster puis ffuf.
     - Nettoyage :
         * suppression des séquences ANSI,
         * réduction des URLs complètes au chemin (/path),
         * normalisation (suppression des / finaux, déduplication, conservation de la casse).
     - Calcul :
         * du nombre total de chemins bruts (ferox + ffuf, après filtrage),
...
Résultats :
  - Affichage à l’écran avec sections détaillées :
      ...
      * Résultats dirsearch (placeholder désactivé).
...
--debug               Affiche les commandes ffuf/ferox et conserve les fichiers /tmp
...
La version courante du script mon-nouveau-recoweb est v1.5.0
