...
# --- PaperMod / navigation ---
type: "writeups"
summary: "Machine HTB Easy : SSRF sur une fonctionnalité d’upload, API interne exposée, accès SSH puis escalade via sudo et GitPython."
description: "Writeup Editorial HTB Easy : exploitation SSRF, découverte d’une API interne, accès SSH puis escalade Linux via sudo et GitPython."
tags: ["Hack The Box","HTB Easy","linux-privesc","Web","SSRF","API","SSH","sudo","GitPython","Git"]
...
cover:
  image: "image.png"
  alt: "Machine Editorial HTB Easy exploitée via SSRF, API interne exposée et escalade de privilèges Linux avec sudo et GitPython"
  caption: ""
  relative: true
  hidden: false
  hiddenInList: false
...
ctf:
  ...
  skills: ["Enumeration","Web","SSRF","API Enumeration","SSH","sudo","GitPython","Privilege Escalation"]
  time_spent: "2h"
  # vpn_ip: "10.10.14.xx"
...
L’exploitation consiste donc à fournir une fausse URL Git qui lance notre propre script.
...
- `chmod 4755` ajoute le bit SUID. Le premier chiffre, `4`, correspond au bit SUID. Il indique que le programme doit s’exécuter avec les droits de son propriétaire, ici `root`;
- comme la copie est effectuée par une commande exécutée avec les privilèges `root`, le fichier `/tmp/rootbash` est créé avec `root` comme propriétaire ;
