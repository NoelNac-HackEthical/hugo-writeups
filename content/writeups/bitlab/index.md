...
title: "Bitlab — HTB Medium Writeup & Walkthrough"
linkTitle: "Bitlab"
slug: "bitlab"
date: 2026-09-18T10:57:43+02:00
#lastmod: 2026-09-18T10:57:43+02:00
draft: true
...
Le scan TCP complet (`scans_nmap/bitlab/full_tcp_scan.txt`) montre les ports ouverts suivants :

```bash
nmap -sCV -p- -T4 -oN scans/nmap_full.txt bitlab.htb
```
...
Les résultats sont enregistrés dans (`scans_nmap/bitlab/enum_ftp_smb_scan.txt`) :
...
Les résultats sont enregistrés dans (`scans_nmap/bitlab/aggressive_vuln_scan.txt`) :

```bash
 nmap -Pn -A -sV -p"22,2222,8080,35627,42277" --script="http-vuln-*,http-shellshock,http-sql-injection,ssl-cert,ssl-heartbleed,sslv2,ssl-dh-params" --script-timeout=30s -T4 "bitlab.htb"
```
...
Le script exécute ensuite un scan ciblé CMS (`scans_nmap/bitlab/cms_vuln_scan.txt`).



### Scan UDP rapide

Le script lance également un scan UDP rapide afin de détecter d’éventuels services supplémentaires (`scans_nmap/bitlab/udp_vuln_scan.txt`).
...
```bash
mon-recoweb bitlab.htb

# Résultats dans le répertoire scans_recoweb/bitlab
#  - scans_recoweb/bitlab/RESULTS_SUMMARY.txt     ← vue d’ensemble des découvertes
#  - scans_recoweb/bitlab/dirb.log
#  - scans_recoweb/bitlab/dirb_hits.txt
#  - scans_recoweb/bitlab/ffuf_dirs.txt
#  - scans_recoweb/bitlab/ffuf_dirs_hits.txt
#  - scans_recoweb/bitlab/ffuf_files.txt
#  - scans_recoweb/bitlab/ffuf_files_hits.txt
#  - scans_recoweb/bitlab/ffuf_dirs.json
#  - scans_recoweb/bitlab/ffuf_files.json
