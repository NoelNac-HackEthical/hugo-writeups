...
cover:
  image: "image.png"
  alt: "Machine Bashed HTB Easy exploitée étape par étape via un shell PHP, sudo et une escalade avec Bash SUID."
  caption: ""
  relative: true
  hidden: false
  hiddenInList: false
...
- passer de `www-data` à `scriptmanager` via sudo ;
- comprendre le rôle du répertoire `/scripts` ;
- exploiter l’exécution automatique d’un script par `root` ;
- terminer l’escalade avec un Bash SUID.
